...
// the basic structure for a PLane, the plane normal and the distance of the plane from the center d
struct plane{
	glm::vec3 n;
	float d;
};
...
float intersectSegmentPlane(glm::vec3 A, glm::vec3 B, plane P)
{
	...
	if (t > 1.0f)										// If the point lies farther than the point B i.e. outside the line segment; return the maximum possible value.
		return FLT_MAX;
	else
		if (t < 0.0f)
			return -FLT_MAX;							// if the point lies behind the point A i.e. before the line segment even starts, return the lowest possible value.
...
bool is_colliding()
{
	...
	for (int i = 0; i < 3; i++)			// for all the 3 planes: conisdering near and far as on plane as thet are parallel to each other.
	{
...
void setup()
{
	...
	line.point1 = glm::vec3(0.0f, 0.0f, 0.0f);
	line.point2 = glm::vec3(1.0f, 1.0f, 1.0f);
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "AABB-Line Segment 3D", nullptr, nullptr);

	std::cout << "\n This is a collision test between a line segment and a Axis aligned bounding box \n in 3D.\n\n\n\n\n";
